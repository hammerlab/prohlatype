...
let add expected_allele mp e =
  ...
  | (v, (al, Three (a, b, _)))
  | (v, (al, Four (a, b, _, _))) ->
      if al <> expected_allele then
        invalid_argf "Found different allele %s than expected: %s" al expected_allele
      else
        match M.find (a, b) mp with
        | exception Not_found -> M.add (a,b) (float_of_string v) mp
        | _current_value      -> mp

let compress expected_allele fname =
  let ic = Scanf.Scanning.from_file fname in
  let add = add expected_allele in
  let rec loop mp =
    try 
      let p =
