...
let pos_to_string pm =
  Partition_map.Pm.to_string pm
    (function | None   -> "none"
              | Some c -> sprintf "some %c" (Base.to_char c))

(* let setup () =
  let a = Alleles.Input.AlignmentFile ("../foreign/IMGTHLA/alignments/A_gen.txt", true) in
  let mp, merge_map = Alleles.Input.construct a |> unwrap_ok in
  let ba, pmap = initialize_base_array_and_position_map mp.Mas_parser.ref_elems in
  let arr = init_state ba in
  let l al =
      Option.value_exn (List.Assoc.get al mp.Mas_parser.alt_elems) ~msg:""
  in
  mp, merge_map, ba, arr, pmap, l,
    fun al ->
      add_alternate_allele pmap al (l al) ba arr *)
...
module ForwardMultipleGen (R : Ring) = struct
  ...
  let cam_max =
    Pm.fold ~init:R.zero ~f:(fun m _s v -> R.max m v)

  let cam_max_cell =
    Pm.fold ~init:R.zero ~f:(fun m _s c -> R.max m c.match_)
