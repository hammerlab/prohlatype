...
let compute_mismatches ?(search_pos_start=0) {g; aindex; bounds} search_seq pos =
  ...
  let assign_mismatches_for_node ?starting_with mismatches node =
    let init =
      match starting_with with
      | Some s -> s
      | None   -> Alleles.Set.empty ()
    in
...
let thread_through_graph ~mub g search_seq search_pos_start pos =
  let open Nodes in
  let open Graph_index in
  let align_against_seq = align_against_seq search_seq in
  let rec descend node m0 ~search_pos =
    G.fold_succ_e (fun (_, edge, vs) acc ->
      ...
      g node []
  in
  let { sequence; offset; _ } = pos in
  match align_against_seq ~search_pos:search_pos_start ~node_seq:sequence ~node_offset:offset with
  | `Finished mismatches           ->
      if mismatches > mub then None else
        Some (Leaf mismatches)
  | `GoOn (mismatches, search_pos) ->
      if mismatches > mub then None else
        let node = N (pos.alignment, pos.sequence) in
        Some (Partial (descend node mismatches ~search_pos))

let align ?(mub=max_int) g index search_seq =
  let open Graph_index in
  let km1 = k index - 1 in
  starting_with index search_seq >>= (fun lst ->
      Ok (List.filter_map lst ~f:(thread_through_graph ~mub g search_seq km1)))
